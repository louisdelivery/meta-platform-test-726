...
export class WebhookController {
  ...
  @ApiOperation({
    summary:
      "Challenge Meta: valide hub.verify_token puis renvoie hub.challenge",
  })
  verify(@Query() query: WebhookChallengeDto, @Res() res: Response) {
    ...
    if (
      mode === "subscribe" &&
      token === this.config.get<string>("meta.verifyToken")
    ) {
  ...
  @ApiBody({
    schema: { example: { object: "whatsapp_business_account", entry: [] } },
  })
  @ApiOperation({
    summary:
      "Recoit tous les webhooks Meta, les stocke, puis repond 200 rapidement",
  })
  async create(
    @Req() req: Request,
    @Headers() headers: Record<string, string>,
    @Body() payload: WebhookIngestDto,
  ) {
