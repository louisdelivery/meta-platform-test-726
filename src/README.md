...
`app` assemble l'application, `config` centralise les variables, `database` expose la persistance, `prisma` encapsule le client Prisma, et chaque domaine WhatsApp/Meta possede son module NestJS dedie.

Chaque feature suit la convention 1HandBiz :
...
Les controllers restent minces, les validations utilisent Joi avec `JoiValidationPipe`, les services portent la logique métier, et les réponses sortantes passent par un mapper.
